...
import { listLatestSavedProviderQuotes } from "@/lib/portfolio-quotes";
...
export async function GET(request: Request) {
  ...
  try {
    const [settings, transactions, reports, savedQuotes] = await Promise.all([
      getOrCreateSettings(auth.ownerEmail),
      listTransactions(auth.ownerEmail, 2_000),
      listResearchRuns(auth.ownerEmail, 5),
      listLatestSavedProviderQuotes(auth.ownerEmail),
    ]);
